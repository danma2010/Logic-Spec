...
Antigravity. The agent generates each design (vendor IP config, custom RTL,
testbenches) and the scripts to build/simulate it. **The user runs Questa and
Vivado manually** by executing the generated scripts. The agent never runs EDA tools.
...
4. **EDA execution is manual.** The agent produces scripts (Vivado Tcl, Questa
...
| `fpga-critic` | isolated simulation failure & waveform diagnosis | targeted diagnosis, root cause |
